...
8. Before a raw `graph.add_node` mutation, call `graph_cmd(command="describe_node", ...)` for the target Blueprint. Use the returned contract to select accepted parameters and pin names; correct validation failures before retrying.
