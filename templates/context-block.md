...
Editor lifecycle: load the `cortex-editor` skill to start/verify the UE editor and `cortex-status` for MCP/domain health. After init or structural content changes, load `cortex-schema-refresh` to regenerate `.cortex/schema/`. Agents may use `editor_cmd` for editor/PIE interaction and `core_cmd` for asset lifecycle operations.
