...
`list_animation_bindings` returns canonical `FWidgetAnimationBinding` records, possessables, tracks, channels, and a scoped `domain_signature` fingerprint. `remove_animation_binding` surgically removes a single binding record by selector (`{widget_name, slot_widget_name, binding_guid, is_root_widget}`) using optimistic locking (`expected_fingerprint`), defaulting to `dry_run=true` and `save=false`. Shared MovieScene possessables and tracks are preserved.
